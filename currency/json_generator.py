...
class JsonGenerator:
    ...
    def create_json(self, currency_from, currency_to, amount):
        ...
        if currency_to == "ALL":
            result_currency_to = self.get_all_currencies(currency_from, amount)
        else:
            result_currency_to = {
                currency_to: self.converter.convert_from_to(currency_from, currency_to) * float(amount)
            }
    ...
    def get_all_currencies(self, currency_from, amount):
        ...
        for currency_to in self.converter.currency_data.keys():
            ...
            result[currency_to] = self.converter.convert_from_to(currency_from, currency_to) * float(amount)
